...
class InpaintGenerator(BaseNetwork):
    def __init__(self, args):  # 1046
        ...
        # Use a static ModuleList for the middle blocks
        self.middle = nn.Sequential(*[AOTBlock(256, args.rates) for _ in range(args.block_num)])
...
class UpConv(nn.Module):
    ...
    def forward(self, x):
        # Use self.scale here for consistency
...
class AOTBlock(nn.Module):
    def __init__(self, dim, rates):
        super(AOTBlock, self).__init__()
        # Replace dynamic attribute creation with a ModuleList
    ...
    def forward(self, x):
        # Process each block in the ModuleList
